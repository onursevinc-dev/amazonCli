            ...
            <div class="nav-global-location">
              <nuxt-link v-if="$auth.$state.loggedIn" to="/address" class="nav-a nav-a-2">
                ...
                <div id="glow-ingress-block">
                  ...
                  <span v-if="$auth.$state.user.address != null" class="nav-line-2" id="glow-ingress-line2">{{$auth.$state.user.address.city}}</span>
                  <span v-else class="nav-line-2" id="glow-ingress-line2">Please Select Address</span>
                </div>
              </nuxt-link>
              <nuxt-link v-else to="/login" class="nav-a nav-a-2">
                <div class="nav-sprite" id="nav-packard-glow-loc-icon"></div>
                <div id="glow-ingress-block">
                  <span class="nav-line-1" id="glow-ingress-line1">Deliver to</span>
                  <span class="nav-line-2" id="glow-ingress-line2">Please Login</span>
                </div>
              </nuxt-link>
            </div>
          </div>
          ...
          <div class="col-xl-4 col-lg-5 col-md-6 col-sm-4 pl-0">
            <div class="nav-tools">
              <a href="#" id="icp-nav-flyout" class="nav-a nav-a-2 icp-link-style-2">
                <span class="icp-nav-link-inner">
                  <span class="nav-line-1">
                    ...
                    <span class="icp-nav-language">TR</span>
                  </span>
                  <span class="nav-line-2">
                    &nbsp;
                    <span class="nav-icon nav-arrow" style="visibility: visible"></span>
              ...
              <nuxt-link to="/cart" class="nav-a nav-a-2" id="nav-cart">
                ...
                <span id="nav-ewc-cart-count" aria-hidden="true"
                      class="nav-cart-count nav-cart-0">{{getCartLength}}</span>
              </nuxt-link>
            </div>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    async getCategories() {
      let responseCategories = await $axios.$get("/api/categories");
      return {categories: responseCategories.categories}
